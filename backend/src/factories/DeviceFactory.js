class DeviceFactory {
  static createDevice(feed_key) {
    ...
    // Phân loại Sensor
    if (key.includes('sensor') || key.includes('temp') || key.includes('humid') || key.includes('light') || key.includes('lux') || key.includes('motion') || key.includes('pir')) {
      type = 'Sensor';
    }
    ...
    if (key.includes('led') || (key.includes('light') && !key.includes('sensor'))) {
      ...
      type = 'Sensor';
    } else if (key.includes('motion') || key.includes('pir')) {
